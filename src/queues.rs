...
pub struct LocalQueue {
  ...
  pub(crate) thread: RefCell<Option<std::thread::Thread>>,
...
impl LocalQueue {
  ...
  pub fn cpu_usage(&self) -> f64 {
    ...
    let avg =
      f64::from_bits(if avg_bits > (u64::MAX as u128) {
  ...
  fn steal_and_pop(&self) -> Option<Runnable> {
    ...
    if *SMOLSCALE2_ALLOW_SFO && self.stealing_from_others
    {
      ...
      for (id, stealer) in stealers.into_iter() {
          ...
          })
        {
          ...
          GLOBAL_QUEUE.locals.peek_with(&id, |_, lq| {
            lq.backlogs.decr();
          });
    ...
    let to_steal =
      ((global_len / 2) + 1).min(64).min(global_len);
