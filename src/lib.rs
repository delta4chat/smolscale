...
use portable_atomic::{
  AtomicBool, AtomicU128, AtomicUsize, Ordering::Relaxed,
};
...
pub(crate) static THREAD_MAP: Lazy<
  scc::TreeIndex<u128, Arc<std::thread::JoinHandle<()>>>,
> = Lazy::new(scc::TreeIndex::new);
static THREAD_SPAWN_LOCK: Lazy<parking_lot::Mutex<()>> =
  Lazy::new(|| parking_lot::Mutex::new(()));

pub(crate) static MONITOR: OnceCell<
    ...
    let thread = std::thread::Builder::new()
      ...
      .spawn(move || {
        ...
        let fut = async {
          ...
          if exitable {
            new_executor::run_local_queue(Some(
              Duration::from_secs(10),
            ))
            .await;
          } else {
            new_executor::run_local_queue(None).await;
          };
        }
        .compat();
...
impl<T, F: Future<Output = T>> Drop
  for WrappedFuture<T, F>
{
  fn drop(&mut self) {
    ACTIVE_TASKS.decr();
...
impl<T, F: Future<Output = T>> Future
  for WrappedFuture<T, F>
{
  type Output = T;
  ...
  fn poll(
    ...
    // 'self' moved here
    let fut =
      unsafe { self.map_unchecked_mut(|v| &mut v.fut) };
    if *SMOLSCALE_PROFILE {
      ...
      let mut entry =
        PROFILE_MAP.entry(task_id).or_insert_with(|| {
...
impl<T, F: Future<Output = T> + 'static>
  WrappedFuture<T, F>
{
    ...
    WrappedFuture {
      ...
      spawn_btrace: if *SMOLSCALE_PROFILE {
        ...
        PROFILE_MAP
          .insert(
            task_id,
            (bt.clone(), Duration::from_secs(0)),
          )
          .expect("bug: duplicated task id");
        Some(bt)
      } else {
        None
      },
...
/// Profiling map.
static PROFILE_MAP: Lazy<
  scc::HashMap<u128, (Arc<Backtrace>, Duration)>,
> = Lazy::new(|| {
  std::thread::Builder::new()
...
mod tests {
  ...
  fn nested_block_on() {
    assert_eq!(
      1u64,
      block_on(async { block_on(async { 1u64 }) })
    )
  }
}
